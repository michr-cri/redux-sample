    ...
    render() {
        ...
        return (
            <div>
                ...
                <a className={this.props.page===1?"disabled":""} onClick={this.handlePrevClicked.bind(this)} href="javascript:void(0);">Prev</a>
                {pageNums.map((pageNum, index) => <Link key={index} to={{
                    pathname: '/cities',
                    search: '?page='+ pageNum + '&page-size=' + this.props.pageSize
                }} >{pageNum}</Link>)}
                <a className={this.props.page===Math.ceil(this.props.totalCount/this.props.pageSize)?"disabled":""}
